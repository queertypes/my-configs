...
NUM_CORES=`cat /proc/cpuinfo | grep 'cpu core' | cut -d: -f2 | tail -n 1`

export CONCURRENCY_LEVEL="(($NUM_CORES + 1))"
export TERM=xterm

alias m="make -j$(($NUM_CORES + 1))"
alias n="ninja -j$(($NUM_CORES + 1))"
alias c='m clean'
alias x='exit'
alias rmtmp='rm *~'
...
alias net_speed='wget --output-document=/dev/null http://speedtest.wdc01.softlayer.com/downloads/test500.zip'
alias correct_perms='for i in *; do if [ -d "$i" ]; then chmod 755 "$i"; else chmod 644 "$i"; fi; done'
